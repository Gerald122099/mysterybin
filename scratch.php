...
// Fetch data for dashboard - UPDATED to use correct column names
$total_today = $conn->query("SELECT SUM(total_grams) as total FROM transactions WHERE DATE(created_at)=CURDATE()")->fetch_assoc()['total'];
$logs = $conn->query("SELECT *, total_grams as weight, accepted_count as reward_dispensed FROM transactions ORDER BY created_at DESC LIMIT 10");
...
// Analytics data (last 7 days) - UPDATED to use correct column name
$analytics_data = [];
for ($i = 6; $i >= 0; $i--) {
    $date = date('Y-m-d', strtotime("-$i days"));
    $result = $conn->query("SELECT SUM(total_grams) as total FROM transactions WHERE DATE(created_at)='$date'");
                        ...
                        <tbody>
                            <?php while ($log = $logs->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><?php echo $log['reward_dispensed'] ? 'Yes' : 'No'; ?></td>
            ...
            <div class="page-content" id="transactions-page">
                ...
                <div class="filters" style="margin-bottom: 20px; display: flex; gap: 15px;">
                    ...
                    <select id="reward-filter" style="padding: 8px; border-radius: 4px; border: 1px solid #ddd;">
                        ...
                        <option value="1">Reward Given</option>
                ...
                <div class="table-container">
                    <table>
                        ...
                        <tbody>
                            ...
                            $all_logs = $conn->query("SELECT *, total_grams as weight, accepted_count as reward_dispensed FROM transactions ORDER BY created_at DESC");
                            while ($log = $all_logs->fetch_assoc()): ?>
                                <tr>
                                    <td><?php echo $log['transaction_id']; ?></td>
                                    <td><?php echo $log['bin_id']; ?></td>
                                    <td><?php echo $log['weight']; ?></td>
                                    <td><?php echo $log['reward_dispensed'] ? 'Yes' : 'No'; ?></td>
                                    <td><?php echo $log['started_at']; ?></td>
                                    <td><?php echo $log['ended_at']; ?></td>
                                    <td><?php echo $log['created_at']; ?></td>
                                    <td>
